...
> **Scope of this spec = Slice 1 only: happy path + Redis internals.** Fan-out
> (moderation/analytics), resilience (`XAUTOCLAIM`/kill-worker/DLQ) and `OllamaLlmClient` are
> **out of scope here** and captured in *Roadmap* (§ Appendix) for later slices.
...
### Slice 2 — Fan-out (`cg:moderation`, `cg:analytics`)

Same `chat:{cid}`, additional groups reading the **same** entries without copy:
- `cg:moderation` — simple regex/keyword flagging → annotate `chat:{cid}:flags`, `XACK`.
- `cg:analytics` — token/latency metrics (counters, optional `TS.ADD` if RedisTimeSeries present),
  `XACK`.
Demo point: `XINFO GROUPS` shows all three groups consuming independently.
