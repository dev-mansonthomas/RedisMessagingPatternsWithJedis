...
QUICKSTART — paste the indented lines into a terminal (needs Docker + the Rust toolchain):
    ...
    cd blog/dlq-redis-streams/samples/rust && cargo run
...
What read_claim_or_dlq returns (maps 1:1 to the Lua "return { a, b }")
  ...
  Either list may be empty. The `redis` crate returns an untyped redis::Value tree:
  arrays are Value::Array, leaf strings are Value::BulkString(bytes). The helpers
  below (s, arr) unwrap those cases; the rest is walking the tree.
*/
use redis::Value;
use std::env;
